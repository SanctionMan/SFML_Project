...
void State_Intro::OnCreate() {
	...
	bg_texture->isSmooth();
	m_BackgoundSprite.setTexture(*bg_texture);
	m_BackgoundSprite.setPosition(0.0f, 0.0f);
	m_BackgoundSprite.setScale(float(windowSize.x) / bg_texture->getSize().x, float(windowSize.y) / bg_texture->getSize().y);
